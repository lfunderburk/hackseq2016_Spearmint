...
all: \
	results/200k/k$k/s$s/hsapiens-scaffolds.fac.tsv \
...
results/200k/k$k/s$s/hsapiens-scaffolds.fa: data/200k.fq
	mkdir -p $(@D)
	abyss-pe -C $(@D) name=hsapiens k=$k s=$s in=$(realpath $<)
...
results/200k.fac.tsv: \
	results/200k/k*/s*/hsapiens-scaffolds.fac.tsv \
	results/200k/k$k/s$s/hsapiens-scaffolds.fac.tsv
	mlr --tsvlite cat $^ >$@
	datamash -H max N50 <$@
